...
async def generate_text_completion(
    ...
    if target_words:
        ...
        topic_instruction = (
            f"**VOCABULARY FOCUS**: Generate questions that test these specific GRE vocabulary words "
            f"in meaningful contexts. Each question should feature at least one of these words in "
            f"either the passage or answer choices:\n\n" + 
            "\n\n".join(word_details) +
            "\n\nEnsure the vocabulary is used naturally in sophisticated, graduate-level contexts."
        )
    ...
    for difficulty, count in difficulties:
        ...
        for attempt in range(max_retries):
            try:
                if attempt > 0:
                    logger.info(f"Retry attempt {attempt + 1}/{max_retries} after 180s delay")
                ...
                if questions:
                    batch_id = f"text_completion_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{difficulty}"
    ...
    logger.info(f"TC generation complete: {len(all_questions)} questions generated for {len(target_words)} vocabulary words")
